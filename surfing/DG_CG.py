...
savedir = "DG_CG_%i" % num_computation    
...
def LHS():
    ...
    LHS -= inner(dot(avg(sigma(du,alpha)),n('+')), jump(v))*dS + inner(dot(avg(sigma(v,alpha)),n('+')), jump(du))*dS
    LHS += 5 * pen_value/h_avg * inner(avg(a(alpha))*jump(du), jump(v))*dS
    LHS = assemble(LHS)
    bc_u.apply(LHS)
